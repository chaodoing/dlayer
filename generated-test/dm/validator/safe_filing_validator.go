...
type SafeFilingRequest struct {
	Id              int64     `json:"id" form:"id" xml:"id" url:"id" validate:"required|int" label:"备案记录ID"`
	CompanyId       float64   `json:"company_id" form:"company_id" xml:"company_id" url:"company_id" validate:"required|float" label:"关联公司ID（外键→company.id）"`
	...
	FilingNumber    string    `json:"filing_number" form:"filing_number" xml:"filing_number" url:"filing_number" validate:"required|string|maxLen:400" label:"备案号（例：京ICP备12345678号）"`
	FilingSubject   string    `json:"filing_subject" form:"filing_subject" xml:"filing_subject" url:"filing_subject" validate:"required|string|maxLen:800" label:"备案主体（与工商名可能不同）"`
	...
	IsCurrent       int       `json:"is_current" form:"is_current" xml:"is_current" url:"is_current" validate:"int" label:"是否为当前有效备案（用于同一公司多备案场景）"`
	CertificatePath string    `json:"certificate_path" form:"certificate_path" xml:"certificate_path" url:"certificate_path" validate:"required|string|maxLen:2000" label:"证书文件OSS路径"`
	Remark          string    `json:"remark" form:"remark" xml:"remark" url:"remark" validate:"required|string|maxLen:2000" label:"备注（驳回原因/特殊说明）"`
	AuditUserId     float64   `json:"audit_user_id" form:"audit_user_id" xml:"audit_user_id" url:"audit_user_id" validate:"required|float" label:"审核人ID"`
	AuditAt         time.Time `json:"audit_at" form:"audit_at" xml:"audit_at" url:"audit_at" validate:"required" label:"审核时间"`
	CreatedBy       float64   `json:"created_by" form:"created_by" xml:"created_by" url:"created_by" validate:"required|float" label:"创建人"`
}
