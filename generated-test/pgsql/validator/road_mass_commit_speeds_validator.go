...
// RoadMassCommitSpeedsRequest is the request validator for the road_mass_commit_speeds table.
type RoadMassCommitSpeedsRequest struct {
	...
	IndicatorId  int     `json:"indicator_id" form:"indicator_id" xml:"indicator_id" url:"indicator_id" validate:"required|int" label:"关联的路测众测指标数据ID"`
...
func (RoadMassCommitSpeedsRequest) ConfigValidation(v *validate.Validation) {
	v.WithScenes(validate.SValues{
		"insert": []string{"Uid", "CommitId", "IndicatorId", "Longitude", "Latitude", "ZoneCode", "Location", "AvgUpload", "AvgDownload", "PeakUpload", "PeakDownload"},
		"update": []string{"Id", "Uid", "CommitId", "IndicatorId", "Longitude", "Latitude", "ZoneCode", "Location", "AvgUpload", "AvgDownload", "PeakUpload", "PeakDownload"},
		"delete": []string{"Id"},
	})
}

// Messages defines Chinese validation messages.
func (RoadMassCommitSpeedsRequest) Messages() map[string]string {
	return validate.MS{
		"float":    "{field}必须是浮点数",
		"int":      "{field}必须是整数",
